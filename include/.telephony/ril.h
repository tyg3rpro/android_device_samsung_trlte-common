...
/* Used by RIL_REQUEST_SIM_TRANSMIT_APDU_CHANNEL andRIL_REQUEST_SIM_TRANSMIT_APDU_BASIC. */
typedef struct {
...
typedef enum {
    ...
    PDP_FAIL_OEM_DCFAILCAUSE_0x1004,
...
typedef struct {
    int signalStrh;  /* Valid values are (0-31, 99) as defined in TS 27.007 8.5 */
 ...
 *
 * "response" is int *
 * ((int *)response)[0] is the number of retriremaining, or -1 if unknown
 *
 * Valid errors:
 *
 *  SUCCESS
 ...
 * "data" is NULL
 * "response" is int *
 * ((int *)data)[0] is "n" parameter from TS 27.07.7
 ...
 * ((inta)[0] is == 0 for notifications disabled
 *
 * "response" is NULL
 *
 * Valid errors:
 ...
 * If the chem is valid the 20 digit AKEY is written to NV,
 ...
 *
 * Requests to send a SAT/USAT envelope command to SIM.
 * The SAT/USAT envelope command refers tPP TS 11.14 and 3GPP TS 31.111.
 *
 ...
 *
 * Start Link Capacity EstimateE) service if supported by the radio.
 *
 * "data" is const int *
 ...
 *
 * Callee will invoke the following requests on main threa:
 *
 * RIL_REQUEST_VOICE_REGISTRATION_STATE
 * RIL_REQUEST_OPERATOR
 *
 ...
 * This eventwill be sent in the following scenarios
...
#defe RUL_UNSOL_SIM_COUNT_MISMATCHED 11058
